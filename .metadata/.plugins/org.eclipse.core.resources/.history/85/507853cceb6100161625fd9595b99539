...
public class DoorExecution
{
	...
	{		
		...
		try
		{
			for(i = 1; i<(numberOfDoors+1); i++)// first loop will go from start to end(1-100)
			{
				...
				while(j <(numberOfDoors+1))
				{
					...
					j=j+i; //This will decide the next value for each case. in case of 1 it will be 2, for 6 it will be 12 and so on
				}
			}
		}
