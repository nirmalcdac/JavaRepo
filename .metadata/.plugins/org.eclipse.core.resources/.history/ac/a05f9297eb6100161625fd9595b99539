public class AdvancedEncryptionStandard
{
	public static void main(String[] args) 
	{
		try
		{
			String plaintext = "ramdaskarmali";
			String key = "ramdaskarmaliramdaskarmaliramdas";
			System.out.println("origional text : "+plaintext);
			byte[] ciphertext;
			ciphertext =encrypt(plaintext.getBytes(), key.getBytes());
			System.out.println("ciphertext     : "+new String(ciphertext));		
		}
		catch(Exception e)
		{
			e.printStackTrace();
		}
	}
	
	//Blocksize=block size (in words): no of columns in state (fixed at 4 for AdvancedEncryptionStandard)
	//Keylength=key length (in words): 4/6/8 for 128/192/256-bit keys. In our case it's 4.
	//TotalRounds=no of rounds: 10/12/14 for 128/192/256-bit keys
	/*
	 * Blocksize = 4;
		Keylength = key.length/4;
		TotalRounds = Keylength + 6;
	 */
	
	static int Blocksize, Keylength, TotalRounds;
	static byte[][] Subkeys;
	static int[] substitution_box = 
	{ 
		 
	};
	static int round_constant[] = 
	{ 
		 
	};	
	private static byte[] xor_func(byte[] a, byte[] b) 
	{
		//System.out.println("nirmal1");
		byte[] out = new byte[a.length];
		for (int i = 0; i < a.length; i++) 
		{
			out[i] = (byte) (a[i] ^ b[i]);
		}
		return out;
	}
	private static byte[][] generateSubkeys(byte[] key) 
	{
		//System.out.println("nirmal2");
		byte[][] tmp = new byte[Blocksize * (TotalRounds + 1)][4];
		int i = 0;
		while (i < Keylength) 
		{
			tmp[i][0] = key[i * 4];
			tmp[i][1] = key[i * 4 + 1];
			tmp[i][2] = key[i * 4 + 2];
			tmp[i][3] = key[i * 4 + 3];
			i++;
		}
		i = Keylength;
		while (i < Blocksize * (TotalRounds + 1)) 
		{
			byte[] temp = new byte[4];
			for(int k = 0;k<4;k++)
				temp[k] = tmp[i-1][k];
			if (i % Keylength == 0) 
			{
				temp = SubWord(rotateWord(temp));
				temp[0] = (byte) (temp[0] ^ (round_constant[i / Keylength] & 0xff));
			}
			else if (Keylength > 6 && i % Keylength == 4) 
			{
				temp = SubWord(temp);
			}
			tmp[i] = xor_func(tmp[i - Keylength], temp);
			i++;
		}
		return tmp;
	}

	private static byte[] SubWord(byte[] in) 
	{
		//System.out.println("nirmal3");
		byte[] tmp = new byte[in.length];
		for (int i = 0; i < tmp.length; i++)
			tmp[i] = (byte) (substitution_box[in[i] & 0x000000ff] & 0xff);
		return tmp;
	}
	private static byte[] rotateWord(byte[] input) 
	{
		//System.out.println("nirmal4");
		byte[] tmp = new byte[input.length];
		tmp[0] = input[1];
		tmp[1] = input[2];
		tmp[2] = input[3];
		tmp[3] = input[0];
		return tmp;
	}
	private static byte[][] AddRoundKey(byte[][] state, byte[][] w, int round) 
	{
		//System.out.println("nirmal5");
		byte[][] tmp = new byte[state.length][state[0].length];
		for (int c = 0; c < Blocksize; c++) 
		{
			for (int l = 0; l < 4; l++)
				tmp[l][c] = (byte) (state[l][c] ^ w[round * Blocksize + c][l]);
		}
		return tmp;
	}
	private static byte[][] SubBytes(byte[][] state) 
	{
		//System.out.println("nirmal6");
		byte[][] tmp = new byte[state.length][state[0].length];
		for (int row = 0; row < 4; row++)
			for (int col = 0; col < Blocksize; col++)
				tmp[row][col] = (byte) (substitution_box[(state[row][col] & 0x000000ff)] & 0xff);
		return tmp;
	}
	private static byte[][] ShiftRows(byte[][] state) 
	{
		//System.out.println("nirmal7");
		byte[] t = new byte[4];
		for (int r = 1; r < 4; r++) 
		{
			for (int c = 0; c < Blocksize; c++)
				t[c] = state[r][(c + r) % Blocksize];
			for (int c = 0; c < Blocksize; c++)
				state[r][c] = t[c];
		}
		return state;
	}
	private static byte[][] MixColumns(byte[][] s)
	{
		//System.out.println("nirmal8");
		int[] sp = new int[4];
	    byte b02 = (byte)0x02, b03 = (byte)0x03;
	    for (int c = 0; c < 4; c++) 
	    {
	         sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ s[2][c]  ^ s[3][c];
	         sp[1] = s[0][c]  ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ s[3][c];
	         sp[2] = s[0][c]  ^ s[1][c]  ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
	         sp[3] = FFMul(b03, s[0][c]) ^ s[1][c]  ^ s[2][c]  ^ FFMul(b02, s[3][c]);
	         for (int i = 0; i < 4; i++) s[i][c] = (byte)(sp[i]);
	    }  
	    return s;
	}
	public static byte FFMul(byte a, byte b) 
	{
		//System.out.println("nirmal9");
		byte aa = a, bb = b, r = 0, t;
		while (aa != 0) 
		{
			if ((aa & 1) != 0)
			r = (byte) (r ^ bb);
			t = (byte) (bb & 0x80);
			bb = (byte) (bb << 1);
			if (t != 0)
			bb = (byte) (bb ^ 0x1b);
			aa = (byte) ((aa & 0xff) >> 1);
		}
		return r;
	}
	public static byte[] encryptBloc(byte[] in) 
	{
		//System.out.println("nirmal10");
		byte[] tmp = new byte[in.length];
		byte[][] state = new byte[4][Blocksize];
		for (int i = 0; i < in.length; i++)
			state[i / 4][i % 4] = in[i%4*4+i/4];
		state = AddRoundKey(state, Subkeys, 0);
		for (int round = 1; round < TotalRounds; round++) 
		{
			state = SubBytes(state);
			state = ShiftRows(state);
			state = MixColumns(state);
			state = AddRoundKey(state, Subkeys, round);
		}
		state = SubBytes(state);
		state = ShiftRows(state);
		state = AddRoundKey(state, Subkeys, TotalRounds);
		for (int i = 0; i < tmp.length; i++)
			tmp[i%4*4+i/4] = state[i / 4][i%4];
		return tmp;
	}
	public static byte[] encrypt(byte[] in,byte[] key)
	{	
		//System.out.println("nirmal11");
		Blocksize = 4;
		Keylength = key.length/4;
		TotalRounds = Keylength + 6;	
		int lenght=0;
		byte[] padding = new byte[1];
		int i;
		lenght = 16 - in.length % 16;				
		padding = new byte[lenght];					
		padding[0] = (byte) 0x80;
		for (i = 1; i < lenght; i++)				
			padding[i] = 0;
		byte[] tmp = new byte[in.length + lenght];		
		byte[] bloc = new byte[16];							
		Subkeys = generateSubkeys(key);
		int count = 0;
		for (i = 0; i < in.length + lenght; i++)
		{
			if (i > 0 && i % 16 == 0) 
			{
				bloc = encryptBloc(bloc);
				System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
			}
			if (i < in.length)
				bloc[i % 16] = in[i];
			else
			{														
				bloc[i % 16] = padding[count % 16];
				count++;
			}
		}
		if(bloc.length == 16)
		{
			bloc = encryptBloc(bloc);
			System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
		}
		return tmp;
	}
}