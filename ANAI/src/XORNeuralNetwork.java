...
public class XORNeuralNetwork {
	...
	public static void main(String[] args) {
		...
		while (!flag) {
			while (i < 4) {
				...
				case 0:
					if (output[0][0] >= 0.0f && output[0][0] <= 0.1f)
						result[i] = true;
					break;
				case 1:
					if (output[0][0] >= 0.9f && output[0][0] <= 1.0f)
						result[i] = true;
				}
	...
	private static void multiply(float[][] inputVector, float[][] weightVector1, int row1, int column1, int column2,
			float intermediate[][]) {
		float sum;
