...
public class AStar {
	static String[] OBSTACLES = new String[] { "0,1-1,1", "1,2-1,3", "1,4-2,4", "2,1-2,2", "2,1-3,1", "2,3-3,3",
			"3,2-3,3", "3,0-4,0" };
	...
	private static void sort(int[][] queue, int length) {
		...
		for (i = 0; i < (length - 1); i++)
			for (j = 0; j < length - i - 1; j++)
				if ((queue[j][3] == UNVISITED) && (queue[j + 1][3] == UNVISITED) && (queue[j][2] > queue[j
						+ 1][2])) /* For decreasing order use < */
				{
					for (k = 0; k < 4; k++)
						temp[k] = queue[j][k];
					for (k = 0; k < 4; k++)
	...
	public static boolean obstacleExists(int currentRow, int currentCol, int newRow, int newCol) {
	...
	private static int calculateCost(int currentRow, int currentCol, int newRow, int newCol) {
		int cost = 0;
		if (Math.abs(newRow - GOALROW) == 1 && newCol == GOALCOL)
			cost = 0;
		else if (Math.abs(newCol - GOALCOL) == 1 && newRow == GOALROW)
			cost = 0;
		else
